"""Read the sp2 files from balder and producing spectra in the air. this one expands to search for uncentered or broad lines"""
...
# Used to truncate flux values later to allow us to see the relative curvature of potential wings
...
star_list = ['sun', 'Arcturus', '122563', '140283', '84937', "140283Karin", "arcturusKarin", "122563_3D", "122563_10th",
             "122563_100th", "122563_0kaulakys", "Arcturus_100th", "Arcturus_10th", "Arcturus_0electrons"
    , "Sun_0kaulakys100electrons", "Arcturus_0kaulakys100electrons", "140283_0kaulakys100electrons",
             "84397_0kaulakys100electrons", "122563_0kaulakys100electrons"]
# 14                             15                              16                                      17                          18
...
735.7726 (4 F)4s 3 F3 (3 F)4sp 3 F3 1.443 -1.020 2 1.99 2 4.814 4.886 4.960 4.854 4.901 +0.054 4.868"""
                ...
                scott_data = """440.9520 (3 P)4s 4 P 32 (3 F)4p 4 D 32 1.231 -2.530 7 3.81 2 4.938 4.923 4.951 4.894 4.956
449.3525a (1 D)4s 2 D 32 (3 F)4p 4 F 52 1.080 -2.780 7 3.18 1 4.897 4.887 4.916 4.862 4.920
...
5418.77 16 b b2D2.5 z2F◦2.5 1.58 51 -2 0.05 1 4.90 4.89 4.90 4.89"""
        ...
        def get_144():
            ...
            data144ti1 = """TiI   3989.757  0.021  -0.198          -0.126      21.37  0.73  2.73  0.02
        ...
        TiI   5210.384  0.048  -0.884          -0.850       5.87  0.11  2.70  0.01"""
            ...
            data144ti2 = """TiII  4028.338  1.892  -0.960          -0.959      12.21  0.20  2.67  0.01
        ...
        Ti ii 5418.77 1.58 −2.11 49.0 4.95 0.02 97.8 4.65 0.04 −0.30 0.04"""
            #6258.1       157.988     3.363 removed
            # 5689.5        76.924     0.700
            datati1ew = """4758.1       115.993     3.332
6258.1       157.988     3.363
4759.3       112.543     1.788
4778.3        84.851     1.277
...
5366.6        75.554     2.440
5384.6        57.245     0.779
5426.3       126.565     1.825
5448.9         0.000     0.000
5449.1         0.000     0.000
5453.6        76.460     2.076
5460.5       145.064     1.039
5465.8        29.690     0.205
5471.2        88.090     0.473
5490.8         0.000     0.000
5514.3         0.000     0.000
...
5937.8        95.440     0.507
5978.5       108.226     0.671
6064.6       104.780     0.555
6091.2        87.629     0.475
6126.2       133.040     0.696
6336.1        77.201     0.419
6395.5        22.009     0.152
6497.7        65.279     0.363
...
5211.5        65.531     1.450"""
        ...
        elif star_number == 1 or star_number == 11 or star_number == 12 or star_number == 13 or star_number == 15:
        ...
        for abundance in abundances_string:
            ...
            # wait why do we not do it for the sun? Karin said so though so... ok.
            # Anyway yeah it replaces the final column of intensity with this new flux list.
            ...
            print("ratio of intensity", sum(iet2[-1]) / sum(iec2[-1]))
            print(out['muz'])
            # exit()
            ...
            continuum = False
            # the ranges for the wavelenght for lower and upper wavelength limits, added/removed from the center point
            range_for_wavelength_upper = 0.2
            range_for_wavelength_lower = 0.2
            lower_val = []
            upper_val = []
            print("lines", lines)
            for x in range(len(lines)):
                range_for_wavelength_upper = 0.2
                range_for_wavelength_lower = 0.2
                print("working on", lines[x])
                continuum = False
                while continuum == False:

                    upper_val_line = lines[x] + range_for_wavelength_upper
                    lower_val_line = lines[x] - range_for_wavelength_lower
                    # Increases the wavelength range if there are fewer than 8 points in it. Prevents errors if we try to
                    # take the first 3 values of an empty array.
                    while len(air_wave[np.where(np.logical_and(air_wave < upper_val_line, air_wave > lower_val_line))]) < 8:
                        range_for_wavelength_upper += 0.05
                        range_for_wavelength_lower += 0.05
                        upper_val_line = lines[x] + range_for_wavelength_upper
                        lower_val_line = lines[x] - range_for_wavelength_lower

                    print("up low", upper_val_line, lower_val_line)
                    print("mid", lines[x])
                    lindex = np.where(np.logical_and(air_wave < upper_val_line, air_wave > lower_val_line))
                    ...
                    if np.all(limy == 1.):
                        print("No line at", lines[x])
                        exit()
                        break

                    # replace all 1s with the next highest value or our janky method doesn't work. Shouldn't make a difference.
                    non_one = limy[np.where(limy<1.)]

                    max_non_one = max(non_one)
                    limy[np.where(limy==1.)] = max_non_one
                    # The flux values on the far left and right of the line
                    upper_y = limy[0:5]
                    lower_y = limy[-5:]
                    upper_x = limx[0:5]
                    lower_x = limx[-5:]
                    ...
                    # Find how many orders of magnitude until thje flux points deviate. This means we can see a large
                    # gradient when there's been no peak yet. Weak when we see no line ever.
                    scaler = 0
                    while int(maxflux) == int(minflux):
                        print("max, min", int(maxflux), int(minflux))
                        scaler += 1
                        print("scaler", scaler)
                        # If it reaches this much, there's no line to be found. Its just continuum
                        if scaler > 15:
                            print("Scaled too high. Just continuum")
                            exit()
                            break
                        maxflux *= 10
                        minflux *= 10
                    rounded = truncate(min(limy), scaler - 1)
                    print("rounded", truncate(min(limy), scaler - 1))
                    print(scaler)
                    # We've got the exponential and number of decimal places until the max and min values of flux diverge
                    modified_flux = (limy - rounded)
                    modified_flux = modified_flux/(10**-scaler)
                    # Now we have the gradient. Much smaller when we see a large range of flux (as it implies we've
                    # see the line)
                    gradient_modified_flux_lower = linregress(limx[-3:], modified_flux[-3:])[0]
                    gradient_modified_flux_upper = linregress(limx[:3], modified_flux[:3])[0]
                    print("lower gradient (left)", gradient_modified_flux_lower)
                    print("upper gradient (right)", gradient_modified_flux_upper)

                    print(linregress(lower_x, lower_y)[0]/min(limy))
                    print(linregress(upper_x, upper_y)[0]/min(limy))
                    if len(limx)<8:
                        range_for_wavelength_upper += 0.05
                        range_for_wavelength_lower += 0.05
                        print("upped")
                    elif min(limy) > 0.999999:
                        range_for_wavelength_upper += 0.05
                        range_for_wavelength_lower += 0.05
                    elif abs(gradient_modified_flux_upper) > 0.5 and abs(gradient_modified_flux_lower) > 0.5:
                        range_for_wavelength_upper += 0.05
                        range_for_wavelength_lower += 0.05
                        print("upped due to flux")

                    elif abs(gradient_modified_flux_upper) > 0.5:
                        range_for_wavelength_upper += 0.05
                        print("upped due to flux")

                    elif abs(gradient_modified_flux_lower) > 0.5:
                        range_for_wavelength_lower += 0.05
                        print("upped due to flux")
                        ...
                        lower_val.append(lower_val_line)
                        upper_val.append(upper_val_line)
                        print( lower_val, upper_val)
                        print("flux", limy)
                        print("FINISHED ON", lines[x], "\n\n\n")
                        continuum = True
            ...
            for lower_val, upper_val in zip(lower_val, upper_val):
                ...
                isExist = os.path.exists(
                    r"D:\PhD\TiFeAtoms\Jack\balder\Balder_outputs\graphs\\" + star + "\spectra\\" + abundance)
                if not isExist:
                    os.makedirs(
                        r"D:\PhD\TiFeAtoms\Jack\balder\Balder_outputs\graphs\\" + star + "\spectra\\" + abundance)
                ...
                plt.savefig(
                    r"D:\PhD\TiFeAtoms\Jack\balder\Balder_outputs\graphs\\" + star + "\spectra\\" + abundance + "\\" + k + "line" + str(
                        int(np.round(lower_val + 0.1))) + ".png")
                plt.close()
                plt.cla()
                plt.clf()
                count += 1
        ...
        if star == "140283":
            plt.plot(wavelength_dict[abundances_string[0]], equivwidthlist, "--", color="red",
                     label="Literature abundance")
        if star_number == 4 or star_number == 5 or star_number == 2 or star_number == 1 or star_number == 9:
            plt.plot(wavelength_dict[abundances_string[0]], equivwidthlist, "--", color="red",
                     label="Literature abundance")
        ...
        if lte:
            if ti2:
                ...
                pickle.dump(abund_dict, open(
                    r"D:\PhD\TiFeAtoms\Jack\balder\Balder_outputs\\" + star + "\\" + k + "EquivWidths_AbundancesLTE.pkl",
                    "wb"))
        else:
            if ti2:
                pickle.dump(abund_dict, open(
