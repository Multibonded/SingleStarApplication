...
from astropy.io import fits
atomname = r"atom.Ti587_nq2382200.fits"
...
def index_top_transitions():
    ...
    with open(r"Sun_hd84937_rb_weq.txt") as f:
        for line in f:
            # Skip title
            if x==0:
                x += 1
...
levellist = [x for x in range(0,550,50)]
print(levellist)

abundance = "5.00"
f = open('atoms/atom.ti_Nq'+print_freq+'_k'+k_string+"small", 'w')
sys.stdout = f
...
for bb_rad_transition in indexes:  # to convert it to AA from nm we divide by 10
    if int(atom2['jrad'][bb_rad_transition]) not in levellist and int(atom2['irad'][bb_rad_transition]) not in levellist:
        continue
...
for bf_rad_transition in range(len(radiative)):
    ...
    if int(bf_rad_transition) not in levellist and int(radiative[bf_rad_transition]['target']) not in levellist:
        continue
    ...
    for photon in range(0, len(radiative[bf_rad_transition]['wavelength']),100):
...
for x in electrons.keys():
    for y in electrons[x]:
        if x not in levellist and y not in levellist:
            continue
